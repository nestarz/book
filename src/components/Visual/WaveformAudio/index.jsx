  ...
  return (
    <>
      ...
      <WrapperControls>
        {audio}
        <div>
          <StyledButton onClick={() => togglePlay()}>{play ? <IoIosPause /> : <IoIosPlay />}</StyledButton>
          <StyledButton onClick={() => toggleMute()}>{mute ? <IoIosVolumeMute /> : <IoIosVolumeHigh />}</StyledButton>
        </div>
        <span>{Math.round(state.time * 10) / 10}:{Math.round(state.duration * 10) / 10}</span>
      </WrapperControls>
    </>
  )
}
